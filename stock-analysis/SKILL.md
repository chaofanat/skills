...
**目的**：基于当前阶段对应的动态权重，计算五维综合评分，检查各维度之间是否相互印证。

**⚠️ 动态权重机制**：根据第二步的阶段判定结果，使用不同的权重体系。详细规则见 `templates/strategy_rules.md` 中的"综合评分机制（分阶段动态权重）"章节。

| 阶段 | 趋势 | 量价 | 资金 | 乖离 | 动量 | 特殊规则 |
|------|------|------|------|------|------|---------|
| Phase 1 启动期 | 18 | 28 | 24 | 18 | 12 | 背离扣分减半 |
| Phase 2 中期 | 25 | 25 | 20 | 15 | 15 | 无 |
| Phase 3 末期 | 25 | 25 | 20 | 15 | 15 | 末期惩罚+天花板 |

**Phase 3 末期惩罚**：末期得分6-7扣8分(天花板≤70)，8-9扣12分(天花板≤60)，10分扣12分(天花板≤55)
